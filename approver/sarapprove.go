...
type csrRecognizer struct {
	recognize      func(csr *capi.CertificateSigningRequest, x509cr *x509.CertificateRequest, nodeInformer coreinformers.NodeInformer, validationCAPool *x509.CertPool) bool
...
type sarApprover struct {
	...
	validationCAPool *x509.CertPool
}

func NewCSRApprovingController(client clientset.Interface, csrInformer certificatesinformers.CertificateSigningRequestInformer, nodeInformer coreinformers.NodeInformer, validationCAPool *x509.CertPool) *certificates.CertificateController {
	approver := &sarApprover{
		client:           client,
		recognizers:      recognizers(),
		nodeInformer:     nodeInformer,
		validationCAPool: validationCAPool,
	}
...
func (a *sarApprover) handle(csr *capi.CertificateSigningRequest) error {
	...
	for _, r := range a.recognizers {
		if !r.recognize(csr, x509cr, a.nodeInformer, a.validationCAPool) {
			continue
		}
...
func isSelfNodeClientCert(csr *capi.CertificateSigningRequest, x509cr *x509.CertificateRequest, nodeInformer coreinformers.NodeInformer, validationCAPool *x509.CertPool) bool {
	if !isNodeClientCert(csr, x509cr) {
		return false
	}

	nodeName := strings.TrimPrefix(csr.Spec.Username, "system:node:")
	if nodeName == csr.Spec.Username { // TrimPrefix returns the unchanged string when the prefix doesn't exist
		return false
	}

	_, err := nodeInformer.Lister().Get(nodeName) // Check if node still exists
	if err != nil {
		return false
	}
	...
	tlsConfig := &tls.Config{
		RootCAs: validationCAPool,
	}

	for _, ipAddress := range x509cr.IPAddresses {
		validationConn, err := tls.Dial("tcp", fmt.Sprintf("%v:%v", ipAddress.String(), 10250), tlsConfig)
		if err != nil {
			klog.Warningf("Failed to connect to Kubelet for IP validation: %v", err)
			return false
		}
		defer validationConn.Close()
		if err := validationConn.Handshake(); err != nil {
			klog.Warningf("Failed to connect to Kubelet for IP validation: %v", err)
			return false
		}
		foundValidChain := false
		for _, chain := range validationConn.ConnectionState().VerifiedChains {
			if chain[0].Subject.CommonName == csr.Spec.Username {
